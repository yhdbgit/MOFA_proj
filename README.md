...
현재 구현은 프론트엔드와 AI 상담 흐름을 검증하기 위한 MVP입니다. 관리자 웹에는 DB, 로그인, 다중 상담방, WebSocket이 없으며 백엔드 메모리에 저장된 최신 상담 1건을 표시합니다.
...
실제 백엔드 구현 언어는 프론트엔드와 직접적인 의존성이 없습니다. 다음 HTTP 계약을 유지하면 모바일 앱과 관리자 웹의 주소만 실제 서버로 변경해 연결할 수 있습니다.

- 모바일 앱: `POST /chat` 요청 Body와 `{ reply }` 응답
- 관리자 웹: `GET /chat/messages` 응답
- 공문 기능: `/official-documents/draft`, `/official-documents/pdf` 요청·응답

다만 현재 최신 상담 공유 방식은 단일 프로세스 메모리이므로, 실제 백엔드에서는 사용자·상담방 식별자, DB 저장, 실시간 전송 방식과 접근 권한을 별도로 설계해야 합니다.
